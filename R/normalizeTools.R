...
`duffyMagnitudeNormalize` <- function( m, globalSum=nrow(m)*500, verbose=FALSE ) {

	if (verbose) cat( "\nGlobal Magnitude scaling...using logmean...")
	mOut <- m
	for( i in 1:ncol(m)) {
		v <- m[ ,i]
		logv <- log( v, 2)
		logfac <- log( (globalSum/nrow(m)), 2) / mean.default(logv)
		newlogv <- logv * logfac
		newv <- 2 ^ newlogv
		mOut[ ,i] <- newv
	}
...
`rankEquivIntensity` <- function(m, columnSets=list( "all"=1:ncol(m)), verbose=TRUE) {
	...
	for ( l in 1:length( columnSets)) {
		...
		rei <- apply( rankInt[ , useCols], 1, median, na.rm=TRUE)
