...
`plotGeneTimeHourCurve` <- function( gene) {
	...
	ottoData <- ottoSet$ottoData
	whereOtto <- base::match( gene, rownames( ottoData), nomatch=0)
	pOtto <- whereOtto[1]
	if ( pOtto > 0) {
		ottoX <- ottoSet$ottoHours
		ottoY <- ottoData[ pOtto, ]
		ottoY <- (ottoY * ottoSet$ottoSlope) + ottoSet$ottoIntercept
	...
	plot( 10,10, main=paste("Gene Expression Time Profile:    ", gene, "\n", gene2Product(gene)), 
	...
	legendText <- append( legendText, "Otto RNA-seq")
	legend( x="topright", legend=legendText, lwd=3, col=c(2,3,1,4), bg="white")
	return()
}


`getConsensusBestTimePoint` <- function( df, minimumIntensity=1) {
	...
	for ( k in 1:1) {
		...
		ts <- TSset[[j]]
		SCORE <- scoreSet[k]
		ans <- bestTimePoint( df, ts, scaleMethod=SCALE, scoreMethod=SCORE, nTSgenes=N,
...
`evalBestTimePoint` <- function( df, label="", minimumIntensity=1) {
	...
	for ( l in 1:2) {
		...
		LOG <- LOGset[l]
		TS <- names(TSset)[j]
		ans <- bestTimePoint( df, TSset[[j]], scaleMethod=SCALE, scoreMethod=SCORE, useLog=LOG, 
...
`bestTimePoint` <- function( df, tSet, scaleMethod="max", scoreMethod="pearson", useLog=TRUE, nTSgenes=NULL,
				minimumIntensity=1) {
	...
	for ( icol in 1:ncol(df)) {
		...
		if ( base::match( colnames(df)[icol], c("Intensity", "Inten", "INTENSITY", "RPKM_M"),
	...
	whoFound <- ( where > 0)
	if ( ! all( whoFound)) {
		#warning( paste( "bestTimePoint:  missing some TimePoint marker genes: ", sum( !whoFound)))
...
`plotFileSetTimeHourCurves` <- function( fnames, fids, fcolors=1, fgrps="group", fLwd=2, 
	...
	for( i in 1:nF) {
		...
		mydf <- read.delim( f, as.is=T); 
		ans <- getConsensusBestTimePoint( mydf, minimumIntensity=minimumIntensity); 
	...
	legend( "bottomright", myGrps, lwd=4, col=myCols, bg="white", cex=legend.cex)

	return( data.frame( "SampleID"=fids, "BestTimeHour"=timeOut, "BestScore"=scoreOut))
}


`plotMatrixTimeHourCurves` <- function( m, fcolors=1, fgrps="group", fLwd=2, 
				fLty=1, xMax=48, label="label goes here", legend.cex=1.0,
				minimumIntensity=1) {
...
`getGeneFCDTT` <- function( gset, fromHour=10, toHour=16) {
...
`getGeneMFCDTT` <- function( gset, fromHour=1, toHour=30) {
...
`calcExcessFoldOverMFCDTT` <- function( gset, log2fold, fromHour=1, toHour=30) {
	...
	ans <- getGeneMFCDTT( gset, fromHour, toHour)
...
`plotMFCDTTdistribution` <- function( gset="", fromHour=1, toHour=30, foldCutoff=2.0, asLog=TRUE, 
	...
	mfc <- getGeneMFCDTT( allG, fromHour=fromHour, toHour=toHour)
