...
duffy.ROC <- function( goodScores, badScores, label="", visualize=TRUE) {
	...
	if (visualize) {
		...
		isMore <- which( cuts[show] >= ans)
		isLess <- which( cuts[show] < ans)
		rotateAngle <- c( 0, -90)
		posOffset <- c( 4, 1)
		...
		text( mspec[best]+0.1, msens[best]-0.1, paste( "Optimal Yes/No Cutoff:   ", 
				formatC( ans, format=formatType, digits=ndigShow)), 
				pos=4, font=2, cex=1.25, col=2)
	}
