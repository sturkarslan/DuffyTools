...
`table2html` <- function( x, fileout, title="", maxRows=NULL, 
	...
	if ( ! (is.null( linkColumnNames) || is.na( linkColumnNames))) {
	  for ( j in 1:length( linkColumnNames)) {
		...
		plotLinks <- makeGenePlotLinks( gNames, path=linkPaths[j], type="html", extension=linkExtensions[j])
...
makeGenePlotLinks <- function(gNames, path="./png", type="excel", prefix="", suffix="", extension=".png") {

	# create text that excel will treat as a hyperlink to our gene plot
	gSet <- gNames
	fSet <- base::paste( prefix, gSet, suffix, extension, sep="")
	...
	if ( type == "excel") {
		...
		txt <- base::paste("=HYPERLINK(\"", urlSet, "\",\"", gSet, "\")", sep="")
	} else {
		txt <- as.link( urlSet, gSet)
	}
	return( txt)
}
