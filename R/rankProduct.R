...
rankProductDiffExpress <- function( fnames, fids, groupSet, targetGroup=groupSet[1], m=NULL,
	...
	#myRP <- apply( (allRanks/nGenes), MARGIN=1, FUN=prod)
	#myRP <- apply( (allRanks/nGenes), MARGIN=1, FUN=calcRP)
	...
	out <- data.frame( "GENE_ID"=allGenes, "PRODUCT"=gProds, "LOG2FOLD"=myFC,
			"RP_VALUE"=myRP, "AVG_RANK"=avgRank, "AVG_SET1"=avgInten1, 
			"AVG_SET2"=avgInten2, stringsAsFactors=FALSE)
	if ( ! is.null(extraColumn)) {
	...
	colnames(out)[6] <- targetGroup
	lab2 <- paste( "Not", targetGroup, sep=" ")
	colnames(out)[7] <- lab2
	if ( is.null( extraColumn)) {
		out <- out[ ,c(1:5, 8:9, 6:7)]
	} else {
		colnames(out)[8] <- paste( "Extra", targetGroup, sep=" ")
		colnames(out)[9] <- paste( "Extra Not", targetGroup, sep=" ")
		out <- out[ , c( 1:5, 10:11, 6:9)]
	}
	return( out)
}
