...
reduceMatrixToModules <- function( m, geneModules, sampleTraits, gene.names=rownames(m),
	...
	for ( j in 1:NC) {
		...
		lapply( 1:NR, function(i) {
			...
			outN[ i, j] <<- n <- length(v)
			outV[ i, j] <<- average.FUN( v)
			if( n > 1) {
				if ( diff( range( v)) < 0.01) v <- jitter(v)
	...
	return( list( "matrix"=outV, "p.value"=outP))
}
